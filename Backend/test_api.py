...
EXPECTED_EXAMS = {
    "NSEJS": {"exam_date": date(2026, 11, 20), "target_syllabus_percent": 95, "target_problem_count": 1500},
    "NMTC": {"exam_date": date(2026, 10, 15), "target_syllabus_percent": 90, "target_problem_count": 800},
    "IOQM": {"exam_date": date(2026, 9, 8), "target_syllabus_percent": 85, "target_problem_count": 600},
}
EXPECTED_LONG_TERM_DATE = date(2027, 1, 24)
...
def test_solve_endpoint() -> None:
    ...
    if set(data.keys()) != {"result", "explanation", "engine_trace"}:
        fail(
            "/api/solve must return exactly result, explanation, and engine_trace.",
            response_text=response.text,
        )

    result = str(data.get("result", "")).strip()
    explanation = data.get("explanation", [])
    engine_trace = str(data.get("engine_trace", "")).strip()
    if not result:
        fail("/api/solve returned an empty result.", response_text=response.text)
    if not isinstance(explanation, list) or len(explanation) != 4:
        fail("/api/solve returned an invalid explanation payload.", response_text=response.text)
    if not engine_trace:
        fail("/api/solve returned an empty engine_trace.", response_text=response.text)

    lowered_answer = result.lower()
    if "100" not in lowered_answer and "100 meters" not in lowered_answer:
        fail("The result did not contain the expected distance of 100 meters.", response_text=result)

    print("PASS: /api/solve returned HTTP 200")
    print("PASS: /api/solve returned the exact result/explanation/engine_trace payload")
    print("PASS: result contains the expected distance of 100 meters")
    print("\nReturned result:")
    print(result)
