...
class ScholarSolveResponse(BaseModel):
    result: str = Field(..., min_length=1, description="KaTeX-formatted final result string.")
    topics: List[str] = Field(
        default_factory=list,
        description="Short academic concept tags extracted from the problem statement.",
    )
    explanation: List[str] = Field(..., min_length=4, max_length=4, description="Four-stage deterministic explanation.")
    engine_trace: str = Field(..., min_length=1, description="Execution trace string for model routing and verification.")
...
class QueryInput(BaseModel):
    messages: List[Dict[str, Any]] = Field(
        ...,
        min_length=1,
        description="Conversation buffer containing role/content message objects.",
    )
    socratic_mode: bool = Field(
        default=False,
        description="When true, backend should guide with hints instead of full solutions.",
    )
    image_data: Optional[str] = Field(
        default=None,
        description="Optional Base64-encoded image payload for OCR-assisted solving.",
    )
    student_query: Optional[str] = Field(
        default=None,
        min_length=1,
        description="Legacy single-turn question payload (kept for backward compatibility).",
    )
    target_exam: str = Field(default="NSEJS", min_length=1, description="Exam context to tune the explanation style.")
